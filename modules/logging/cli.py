"""
modules/logging/cli.py
------------------------
Logique CLI du module logging d'OpsForge.
Appele via `python main.py logging ...`.
"""
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="opsforge logging",
        description=(
            "Genere une config de collecte/expedition de logs (Fluent Bit "
            "ou Vector), a partir d'un preset ou d'une config JSON."
        ),
    )
    parser.add_argument(
        "config_file",
        nargs="?",
        default=None,
        help="Fichier JSON decrivant sources/destination (voir --preset pour un depart rapide).",
    )
    parser.add_argument(
        "--preset",
        default=None,
    ...
    parser.add_argument(
        ...
        default=None,
        help="Surcharge le backend (fluent-bit / vector).",
    )
    parser.add_argument(
        "-o", "--output-dir",
        default=None,
...
def main(argv=None):
    ...
    try:
        paths = write_logging(config, output_dir)
    except ValueError as e:
        print(f"Erreur : {e}")
        return 1

    print("\nFichier(s) de logging genere(s) avec succes :")
    for path in paths:
        print(f"  - {path}")

    backend = config.get("backend", "fluent-bit")
    if backend == "fluent-bit":
        print(f"\nPour lancer : fluent-bit -c {output_dir}/fluent-bit.conf")
    else:
        print(f"\nPour lancer : vector --config {output_dir}/vector.toml")
